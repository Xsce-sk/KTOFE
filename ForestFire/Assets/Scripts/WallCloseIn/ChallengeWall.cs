...
public class ChallengeWall : MonoBehaviour
{
    // Public Members
    [Header("Challenge Values")]
    public float height = 3f;
    public float minGapSize = 0.6f;
    ...
    IEnumerator StartChallenge()
    {
        ...
        seconds = defaultDuration - (GameManager.game.speed * 0.4f);
        float defaultGapSize = VRBounds.bounds.length / 4f;
        float gapSize = defaultGapSize + minGapSize - (GameManager.game.difficulty * 0.1f * defaultGapSize); // Subtract 10% of the default gap size until it disappears
        ...
        if (_showDebug)
        {
            string debugStatement = "<size=22><b><color=magenta>ChallengeWall</color></b></size>" + "\n";
            debugStatement += "<b><color=cyan>Holding</color></b>" + "\n";
